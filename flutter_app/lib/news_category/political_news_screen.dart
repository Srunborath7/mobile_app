...
class PoliticalNewsScreen extends StatefulWidget {
  ...
  @override
  State<PoliticalNewsScreen> createState() => _PoliticalNewsScreenState();
}
...
class _PoliticalNewsScreenState extends State<PoliticalNewsScreen> {
  ...
  Future<void> fetchTechnologyArticles() async {
    try {
      ...
      final response = await http.get(Uri.parse('$baseUrl/api/categories/article/category/7'));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Political News')),
      body: isLoading
